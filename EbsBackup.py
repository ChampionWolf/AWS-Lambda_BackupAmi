...
def CheckTagsAndCreateSnapshot(ec2, now):
    ...
    for vol in Volumes['Volumes']:
        ...
        snapshotTags = [tag for tag in vol['Tags'] if
                        (tag['Key'] != 'MountPoint') and (tag['Key'] != 'BackupWindowUTC')]
            ...
            if tag['Key'] == 'BackupWindowUTC':
                ...
                for hour in hourList:
                    ...
                    if not hour.isdigit() or int(hour) > 23:
                        print
                        "Value set for tag BackupWindowUTC =", hour, "is Invalid"
                        continue

                    # Create snapshot if current UTC time equals the backup hour in tag
                    if currentHour == int(hour):
                        snapshotDescription = "Created by AWS Lambda Backup Script from %s on %s" % (
                        vol['VolumeId'], str(now.isoformat()))

                        # Create snapshot of the volume
                        snapshotDetails = ec2.create_snapshot(DryRun=False, VolumeId=vol['VolumeId'],
                                                              Description=snapshotDescription)
                        print
                        "Creating snapshot of %s. SnapshotId=%s." % (vol['VolumeId'], snapshotDetails['SnapshotId'])
                        ...
                        print
                        "Current Hour is %s UTC. Backup window of %s is set as %s. Doing nothing." % (
                        currentHour, vol['VolumeId'], tag['Value'])
    return

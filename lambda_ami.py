            ...
            for tag in Instance['Tags']:  # 获取Tag
                    ...
                    for date in AmiDateList:  # 使用Tag的值替换AMI日期的默认值
                        ...
                        if date.isalpha():
                            if date.lower() == 'daily':
                                ...
                                print("Error: Wrong date entry:" + date + ". \
                                Please correct Tag value. Expecting value Daily/[sun-sat]/[Sunday-Saturday]/[1-31] ")
                                continue
                        elif not date.isdigit() or int(date) > 31:  # 验证创建AMI的日期
                            print("Error: Wrong date entry:" + date + ". \
                            Please correct Tag value. Expecting value [1-31]/Daily/[sun-sat]/[Sunday-Saturday]")
                            continue
                        if date == str(today):
                            AmiDate = today
                            break
                ...
                # 如果不是，设置 ‘SkipAmi’标记以跳过此AMI的创建
                    ...
                    for time in AmiTimeList:  # 使用Tag的值替换AMI日期的默认值
                        ...
                        if not time.isdigit() or int(time) > 23:  # 验证创建AMI的时间
                            print("Error: Wrong time entry:" + time + ". \
                            Please correct Tag value. Expecting value [0-23].")
                            continue
                ...
                # 如果指定的设备名称中有任何错误，则不会排除任何设备
                    ...
                    if not tag['Value'].isdigit():
                        print("请注意！实例 {} 拥有无效的 [AmiRetentionDays] 值:[ {} ],\
                        创建的AMI会默认保留 {} 天.".format(InstanceId,tag['Value'],amiRetention))
                    else:
